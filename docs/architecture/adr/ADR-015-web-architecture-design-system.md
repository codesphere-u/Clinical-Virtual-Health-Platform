# ADR-015: Web Architecture & DOCAAS Clinical Design System
...
2. **DOCAAS Clinical Design System (`packages/design-system`)**:
   - Shared Tailwind CSS design plugin with carefully calibrated clinical palette:
     - `docaas-teal` (`#0D746F`): Trust, clarity, primary interactive anchor.
     - `docaas-slate` (`#0F172A`, `#F8FAFC`): Deep readable charcoal text on soothing neutral surfaces.
     - `docaas-rose` (`#E11D48`): High-contrast critical alerts and allergies (never rely on color alone; paired with icons and labels).
