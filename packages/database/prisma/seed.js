...
async function main() {
    ...
    for (const c of cliniciansData) {
        const user = await prisma.user.create({
            data: {
                ...
                role: client_1.UserRole.clinician,
                isMfaEnabled: true,
            },
        });
        const profile = await prisma.clinicianProfile.create({
            data: {
                ...
                compliancePassportStatus: client_1.VerificationStatus.verified,
        ...
        if (i % 3 === 0) {
            await prisma.patientAllergy.create({
                data: {
                    ...
                    severity: client_1.SeverityLevel.life_threatening,
                },
            });
        }
    ...
    const statuses = [
        client_1.AppointmentStatus.consultation_completed,
        client_1.AppointmentStatus.consultation_started,
        client_1.AppointmentStatus.patient_waiting,
    ...
    for (let i = 0; i < 50; i++) {
        ...
        const appt = await prisma.appointment.create({
            data: {
                ...
                status,
                encounterType: client_1.EncounterType.telemedicine_video,
        ...
        if (status === client_1.AppointmentStatus.consultation_completed) {
            ...
            const rx = await prisma.prescription.create({
                data: {
                    ...
                    patientId: patient.id,
                    clinicianId: clinician.id,
                    status: client_1.PrescriptionStatus.authorized,
